...
def config_args(parser):
    ...
    parser.add_argument('--gen_lr', type=float, default=0.002, help='learning rate of generator')
    parser.add_argument('--dis_lr', type=float, default=0.002, help='learning rate of discriminator')
...
if __name__ == '__main__':
    ...
    # build models 
    G = LSTMGenerator(nb_features = args.nb_features, device=device).to(device)
    D = LSTMDiscriminator(nb_features = args.nb_features, device=device).to(device)   
    ...
    dis_optim = torch.optim.Adam(D.parameters(), lr=args.dis_lr) 
    ## scheduler
