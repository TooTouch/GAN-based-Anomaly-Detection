...
class LSTMGenerator(nn.Module):
    ...
    def __init__(self, nb_features, device=None):
        super().__init__()
        self.out_dim = nb_features
        self.device = device

        self.lstm0 = nn.LSTM(nb_features, hidden_size=32, num_layers=1, batch_first=True)
        ...
        self.linear = nn.Sequential(nn.Linear(in_features=128, out_features=self.out_dim), nn.Tanh())
...
class LSTMDiscriminator(nn.Module):
    ...
    def __init__(self, nb_features, device=None):
        ...
        self.lstm = nn.LSTM(input_size=nb_features, hidden_size=100, num_layers=1, batch_first=True)
